...
REM         dist\portfolio_rebalance_setup_v1.1.8.exe（安装包，需装 Inno Setup）
REM ============================================================
setlocal
cd /d "%~dp0.."
...
if exist "%ISCC%" (
    ...
    echo   安装包已生成： %cd%\dist\portfolio_rebalance_setup_v1.1.8.exe
) else (
    where ISCC >nul 2>&1 && ( ISCC build\installer.iss && echo   安装包已生成： %cd%\dist\portfolio_rebalance_setup_v1.1.8.exe ) || (
...
echo   - portfolio_rebalance_setup_v1.1.8.exe   安装包，双击安装（带开始菜单/桌面快捷方式/卸载）
echo.
pause
